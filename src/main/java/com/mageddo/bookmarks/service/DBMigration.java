...
public class DBMigration {
	...
	public static void main(String[] args) {
		...
		new DBMigration(new HikariDataSource(sourceConfig), new HikariDataSource(targetConfig))
		.migrate();
	}
	...
	private void migrateSystemPropertyTable() {
		...
		sourceJdbcTemplate.query("SELECT * FROM SYSTEM_PROPERTY", (rs) -> {
			final StringBuilder sql = new StringBuilder()
				.append("INSERT INTO SYSTEM_PROPERTY ( \n")
				...
				.append(") \n")
				.append(" \n");
			targetJdbcTemplate.update(sql.toString(), new MapSqlParameterSource()
				.addValue("namProperty", rs.getString("NAM_PROPERTY"))
	...
	private void migrateTagBookmarkTable() {
		...
		sourceJdbcTemplate.query("SELECT * FROM TAG_BOOKMARK", (rs) -> {
			final StringBuilder sql = new StringBuilder()
				.append("INSERT INTO TAG_BOOKMARK ( \n")
				...
				.append(" \n");
			targetJdbcTemplate.update(sql.toString(), new MapSqlParameterSource()
				.addValue("idtTag", rs.getLong("IDT_TAG"))
	...
	private Timestamp parseTimestamp(ResultSet rs, String colName) throws SQLException {
		...
		return Timestamp.valueOf(
			LocalDateTime.parse(rs.getString(colName), new DateTimeFormatterBuilder()
